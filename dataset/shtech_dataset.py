...
class SHTechDataset(Dataset):
    def __init__(self, json_path, image_path, text_path, train=True, ext='.jpg'):
        super().__init__()
        ...
        self.image_path = os.path.join(self.image_path, 'training' if self.train else 'testing', 'frames')

        # 设置文本路径
        self.text_path = text_path
        self.text_path = os.path.join(self.text_path, 'training' if self.train else 'testing')
...
class SHTech_Single_Video_Dataset(Dataset):

    '''
    用于单个视频的测试、验证、可视化等作用
    '''

    def __init__(self, video_dir_path, ext='.jpg'):
        super().__init__()
        ...
        # 获取图片帧所在目录及其文件
        self.video_dir_list = os.listdir(video_dir_path)
        self.video_dir_list = sorted(self.video_dir_list)

        # 视频名称
        self.video_name = video_dir_path.split('/')[-1]
        
        # 从第5帧开始取
        self.video_dir_list = self.video_dir_list[4:]

        # 设置图片路径
        self.image_path = '/home/lijuntong/dataset/SHTech/shanghaitech'
        self.image_path = os.path.join(self.image_path, 'training' if self.train else 'testing', 'frames')

        # 设置文本路径
        self.text_path = '/home/datasets/lijuntong_datasets/internvl_video_simsce'
        self.text_path = os.path.join(self.text_path, 'training' if self.train else 'testing')
    ...
    def __getitem__(self, index):
        ...
        frame_name = self.video_dir_list[index].split('.')[0]
        ...
        for frame_name in frame_name_list:
            ...
            frame_path = os.path.join(self.image_path, self.video_name, frame_name + self.ext)
            image = load_image(frame_path, 448)
            images.append(image)

        # 应该改成t-1 的text feature
        text_path = os.path.join(self.text_path, self.video_name, frame_name_list[-2] + '.npy')
        text_feature = np.load(text_path)
...
if __name__ == '__main__':
    # dataset = SHTechDataset(json_path='/home/lijuntong/workplace/internvl_vad/data/shtech_train_debug.json', train=True, ext='.jpg')
    dataset = SHTech_Single_Video_Dataset(video_dir_path='/home/lijuntong/dataset/SHTech/shanghaitech/testing/frames/01_0014')
    dataloader = DataLoader(dataset, batch_size=32, shuffle=True, num_workers=8) # shuffle=False用于验证能否按照视频顺序读取
    print(len(dataset))
